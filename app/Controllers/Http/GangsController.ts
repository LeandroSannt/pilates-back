...
export default class GangsController extends BaseController  {
  constructor(){
    super({service:GangsServices,validator:{create:CreateOrUpdateGangValidator,update:CreateOrUpdateGangValidator}})
  }

 async storeMany({request,response}:HttpContextContract){
    const gangs = new GangsServices()
    const {day,student_id,time} = await request.validate(CreateManyGangStudantValidator)
