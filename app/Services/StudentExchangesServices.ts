...
import {
  default as Exchange,
  default as StudentExchange,
} from "../Models/StudentExchange";
...
export default class StudentExchangeServices extends BaseServices {
  ...
  async getAllExchangesPaginated(
    page: number,
    limit: number,
    student_id: number
  ) {
  ...
  async getAllExchanges() {
    const students = await Student.query().whereExists((query) => {
      query.withScopes((scope) => scope.existsExganges());
    });
  ...
  async cancelGang(
    gang_ids: Array<number>,
    date_lacks: string,
    observation: string
  ) {
    const gangs = await Gang.query()
      .whereIn("id", gang_ids)
    ...
    const result = Promise.all(
      gangs.map(async (gang) => {
        const c = Promise.all(
          gang.studentGang.map(async (student) => {
